...
begin
  ...
  Result.H1 := TStyle.Default.WithModifier([mbBold]).WithFg(clCyan);
  Result.H2 := TStyle.Default.WithModifier([mbBold]).WithFg(clGreen);
  Result.H3 := TStyle.Default.WithModifier([mbBold]).WithFg(clYellow);
  Result.Bold := TStyle.Default.WithModifier([mbBold]);
  Result.Italic := TStyle.Default.WithModifier([mbItalic]);
  Result.Code := TStyle.Default.WithFg(clMagenta);
  Result.Bullet := TStyle.Default.WithFg(clCyan);
  Result.HRule := TStyle.Default.WithFg(clDarkGray);
end;
