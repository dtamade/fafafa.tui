...
begin
  ...
  FMaxVisible := 5;
  FWidth := 30;
  FInfoStyle := TStyle.Default.WithFg(clWhite).WithBg(clBlue);
  FSuccessStyle := TStyle.Default.WithFg(clWhite).WithBg(clGreen);
  FWarningStyle := TStyle.Default.WithFg(clBlack).WithBg(clYellow);
  FErrorStyle := TStyle.Default.WithFg(clWhite).WithBg(clRed);
end;
