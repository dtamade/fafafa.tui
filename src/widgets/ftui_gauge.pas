...
type
  ...
  TGauge = record
    ...
    function WithBlock(const B: TBlock): TGauge;
...
function TGauge.WithBlock(const B: TBlock): TGauge;
begin
  Result := Self;
...
begin
  ...
  begin
    Adv := GraphemeAdvance(Label_[1], LabelLen, LabelIdx);
    if Adv.Width = 0 then
    begin
